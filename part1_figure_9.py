...
ax.set_xscale('log')
ax.set_yscale('log')
ax.text(0.8, 2.25 * 10**-2, r'$C_{\rm th} = 1$\,bits/photon possible for', color='black', horizontalalignment='right')
ax.text(0.8, 2.25 * 10**-3, r'$N_{\rm M} = 0.13$\,noise photons/mode', color='black', horizontalalignment='right')
ax.text(0.8, 2.25 * 10**-4, r'with $M=10^{-5}$, $\eta=0.5$', color='black', horizontalalignment='right')
ax.set_xlabel(r'Capacity (bits/photon)')
ax.set_ylabel(r'Thermal noise $N_{\rm M}$ (photons/mode)')
